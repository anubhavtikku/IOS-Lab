 ...
 */
func progressUpdate(st:Int,goal:Int)
{
    if(st<(goal/10))
    {
    ...
    else if(st<Int(Double(goal)*0.9))
    {
...
progressUpdate(st:20, goal:50)

/*:
 Your fitness tracking app is going to help runners stay on pace to reach their goals. Write a function called pacing that takes four `Double` parameters called `currentDistance`, `totalDistance`, `currentTime`, and `goalTime`. Your function should calculate whether or not the user is on pace to hit or beat `goalTime`. If yes, print "Keep it up!", otherwise print "You've got to push it just a bit harder!"
 */
func almostAddition(currentDistance:Double,totalDistance:Double,currentTime:Double,goalTime:Double)
{
...
almostAddition(currentDistance:10, totalDistance:20, currentTime:5, goalTime:10)
